...
struct Waves: View {
    ...
    var body: some View {
        ZStack {
            ...
            getSinWave(interval: universalSize.width * 1.2, amplitude: 100, baseline: 50 + universalSize.height / 2)
                .foregroundColor(.init(red: 0.3, green: 0.6, blue: 1).opacity(0.4))
                .offset(x: isAnimated ? -1 * (universalSize.width * 1.2) : 0)
                .animation(
                    Animation.linear(duration: 5)
                        .repeatForever(autoreverses: false)
                )
            getSinWave(interval: universalSize.width * 1.2, amplitude: 125, baseline: 25 + universalSize.height / 2)
    ...
    func getSinWave(interval: CGFloat, amplitude: CGFloat = 100, baseline: CGFloat = UIScreen.main.bounds.height/2) -> Path {
        Path { path in
            ...
            path.addLine(to: CGPoint(x: 2 * interval, y: universalSize
                .height))
            path.addLine(to: CGPoint(x: 0, y: universalSize
                .height))
        }
    }
}
